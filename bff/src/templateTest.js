let vm = require("vm")
let templateEngine = require("./tools/TemplateEngine.ts")
let templateMap = {
    templateA: "`<div>${include('templateB')}</div>`",
    templateB: "`<a>haha</a>`"
}


/**
 * 也就是说,会将字符串的内容当作js 语法在独立的环境中执行 ...
 * 所以,一般来说 字符串中都使用模板字符串 进行  代码处理 ..
 * (function(){}) 被称为匿名函数 ...
 */
...
// // 执行模板引擎
let engine = templateEngine(templateMap)
// @ts-ignore
console.log(engine.asText(templateMap['templateA']));
// console.log(engine.include("templateA"));

console.log(vm.runInNewContext("`${user.name}`", {user: {name: "123"}}));


let template1 = "`${user.name}`"
let template2 = "`${template1}`"


let fs = require("fs")
let content = fs.readFileSync(`${__dirname}/template/index.html`)
...
let result = vm.runInNewContext("`` + content", {
    content
})


console.log((function() {
    return `${fs.readFileSync(`${__dirname}/template/index.html`)}`
})())
